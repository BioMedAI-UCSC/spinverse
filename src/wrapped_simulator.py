...
#     return tch.abs(tch.abs(tch.abs(mf_signal["signal_allcmpts"]) / tch.abs(mf_signal["signal_allcmpts"][0, :, 0]).view(1, -1, 1).clamp(min=1e-6))) * 100
...
def wrapped_simulator(femesh_all, setup, faces_prob):
    # Initial input check
