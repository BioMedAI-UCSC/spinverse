...
def solve_mf(femesh, setup, lap_eig, faces_prob=None):
    ...
    for s in range(n_seq):
        ...
        if isinstance(seq, PGSE):
            ...
            chunk_size = 10 # Tune: lower for less memory, higher for speed
            for a in range(n_amp):
                q = q_as[a]
                # Compute K1_a batched over dir
                K1_a = lambda_relax.unsqueeze(0) + 1j * q * A_batch_dir # [n_dir, n_eig, n_eig]
                # Define a wrapper for the exp and matmul (to checkpoint the memory-heavy part)
                def exp_and_mul_forward(K, v, delta):
                    batch = K.shape[0]
                    out = torch.empty_like(v)
                    for i in range(batch):
                        E1_i = torch.linalg.matrix_exp(-delta * K[i])
                        out[i] = torch.matmul(E1_i, v[i])
                    return out
                # v for this amp, batched over dir
                v_a = nu[a] # [n_dir, n_eig, 1]
                # Chunk to reduce batch size for exp
                for d_start in range(0, n_dir, chunk_size):
                    end = d_start + chunk_size
                    K_chunk = K1_a[d_start:end]
                    v_chunk = v_a[d_start:end]
                    # Checkpoint the exp + mul
                    tmp1[a, d_start:end] = checkpoint(exp_and_mul_forward, K_chunk, v_chunk, seq.delta, use_reentrant=False)
            ...
            for a in range(n_amp):
                ...
                # Define a wrapper for the exp and matmul (adjoint)
                def exp_and_mul_adjoint(K_H, tmp, delta):
                    batch = K_H.shape[0]
                    out = torch.empty_like(tmp)
                    for i in range(batch):
                        E1_H_i = torch.linalg.matrix_exp(-delta * K_H[i])
                        out[i] = torch.matmul(E1_H_i, tmp[i])
                    return out
                ...
                for d_start in range(0, n_dir, chunk_size):
                    ...
                    K_H_chunk = K_H_a[d_start:end]
                    tmp2_chunk = tmp2_a[d_start:end]
                    # Checkpoint the exp + mul for adjoint
                    nu_final[a, d_start:end] = checkpoint(exp_and_mul_adjoint, K_H_chunk, tmp2_chunk, seq.delta, use_reentrant=False)
