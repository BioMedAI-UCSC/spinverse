...
def solve_mf(femesh, setup, lap_eig, direction=None, debug_checks: bool = False):
    """
    Matrix-formalism Bloch-Torrey solver (differentiable).
    Computes signal loss for all amplitudes, sequences, and directions.

    Set debug_checks=True to print algebraic diagnostics (commutation, normality, etc.)
    once per sequence (cheap, uses amplitude 0).
    """
    t_start = time.time()
    ...
    # Helper (safe relative norm)
    def _rel(A, B):
        denom = torch.linalg.norm(B)
        return (torch.linalg.norm(A) / (denom + 1e-20)).item()

    # Precompute S once (sequence-independent)
    S = (lambda_mat + relax_mat)  # complex in general but often real-symmetric
    ...
    for s in range(n_seq):
        ...
        # --- Diagnostics: run once per sequence (cheap) ---
        if debug_checks:
            A_dir = A_all[..., 0]                                # [n_eig, n_eig] (n_dir=1)
            a_imax = A_dir.imag.abs().max().item()
            is_Asym  = torch.allclose(A_dir, A_dir.T, atol=1e-6)
            # Commutation ratio
            SA = S @ A_dir
            AS = A_dir @ S
            comm = torch.linalg.norm(SA - AS)
            comm_denom = torch.linalg.norm(S) * torch.linalg.norm(A_dir) + 1e-20
            comm_ratio = (comm / comm_denom).item()

            # Build one K for amplitude 0 (if any) and test properties
            if n_amp > 0 and isinstance(seq, PGSE):
                q0 = q_values[0, s]
                K0 = S + 1j * q0 * A_dir
                is_sym     = torch.allclose(K0.T, K0, atol=1e-6)
                is_herm    = torch.allclose(K0.conj().T, K0, atol=1e-6)
                N = K0 @ K0.conj().T - K0.conj().T @ K0
                normal_ratio = (torch.linalg.norm(N) / (torch.linalg.norm(K0)**2 + 1e-20)).item()

                # Compare eig-similarity vs matrix_exp (should match ~round-off)
                E1_dir = torch.linalg.matrix_exp(-seq.delta * K0)
                evals, evecs = torch.linalg.eig(K0)
                E1_eig = evecs @ torch.diag(torch.exp(-seq.delta * evals)) @ torch.linalg.inv(evecs)
                eig_relerr = _rel(E1_eig - E1_dir, E1_dir)

                # If commuting is (nearly) true, test closed-form in S-basis
                # (diagonalize Hermitian part of S and project A to that basis)
                try:
                    # Make S explicitly Hermitian for eigh
                    S_herm = 0.5 * (S + S.conj().T)
                    svals, Qs = torch.linalg.eigh(S_herm)
                    A_in_S = Qs.conj().T @ A_dir @ Qs
                    # Use only the diagonal of A_in_S for commuting closed form
                    a_diag = torch.diagonal(A_in_S)
                    E1_comm = Qs @ torch.diag(torch.exp(-seq.delta * (svals + 1j * q0 * a_diag))) @ Qs.conj().T
                    comm_relerr = _rel(E1_comm - E1_dir, E1_dir)
                    off = A_in_S - torch.diag(a_diag)
                    off_ratio = (torch.linalg.norm(off) / (torch.linalg.norm(A_in_S) + 1e-20)).item()
                except RuntimeError:
                    comm_relerr = float('nan')
                    off_ratio   = float('nan')

                print(
                    f"[SEQ {s}] A.imag.max={a_imax:.2e}, A symmetric={bool(is_Asym)}, "
                    f"commute ratio={comm_ratio:.2e}, "
                    f"K symmetric={bool(is_sym)}, K Hermitian={bool(is_herm)}, "
                    f"normality ratio={normal_ratio:.2e}, "
                    f"eig-sim relerr={eig_relerr:.2e}, "
                    f"comm-exp relerr={comm_relerr:.2e}, "
                    f"A_in_S offdiag ratio={off_ratio:.2e}"
                )
            else:
                print(
                    f"[SEQ {s}] A.imag.max={a_imax:.2e}, A symmetric={bool(is_Asym)}, "
                    f"commute ratio={comm_ratio:.2e} (no PGSE / no amps, skipped K/E1 checks)"
                )
        # --- end diagnostics ---
