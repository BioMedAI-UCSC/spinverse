...
from src.expm_krylov_diff import expmv_krylov_bd
# from src.expm_krylov import expmv_krylov_bd
...
def solve_mf(femesh, setup, lap_eig, faces_prob=None):
    ...
    for s in range(n_seq):
        ...
        if isinstance(seq, PGSE):
            ...
            # E2 is independent of amp/dir (you can keep this as-is)
            E2 = torch.linalg.matrix_exp(-(seq.Delta - seq.delta) * (lambda_mat + relax_mat))

            # ---- Krylov expm-multiply drop-in (no big E1 tensor) ----
            tmp1 = expmv_krylov_bd(K1, nu, float(seq.delta), m=32, steps=1)            # exp(-δK1) @ nu
            tmp2 = torch.matmul(E2.unsqueeze(0).unsqueeze(0), tmp1)                     # E2 @ (...)
            K1_H = torch.conj(K1).transpose(-1, -2)
            nu_final = expmv_krylov_bd(K1_H, tmp2, float(seq.delta), m=32, steps=1)     # exp(-δK1ᴴ) @ (...)
